            ...
            <td>
                ...
                @if (!empty($result->media_url))
                    <a class="fancybox-buttons" data-fancybox-group="button"
                        href="{{ isset($result->media_url) ? $result->media_url : '' }}"><img height="50" width="50"
                            src="{{ isset($result->media_url) ? $result->media_url : '' }}" /></a>                
                @endif
                @if($result->like_count)
                    <span>{{ $result->like_count }} <i class="ri-thumb-up-line"></i></span>
                @endif
                </div>
            </td>
            ...
            <td>
                <div class="hstack gap-2 flex-wrap">
                    @if ($statusPermission == 1)
                        @if ($result->is_active == 1)
                            <a href='{{ route('admin-InstagramSliders.status', [$result->id, 0]) }}' class="btn btn-danger"
                                id="deactivate-button"><i class="ri-close-line"></i></a>
                        @else
                            <a href='{{ route('admin-InstagramSliders.status', [$result->id, 1]) }}' class="btn btn-success"
                                id="activate-button"><i class="ri-check-line"></i></a>
                        @endif
                    @endif
                    @if ($viewPermission == 1)
                        {{-- <a href="{{route('admin-InstagramSliders.show',base64_encode($result->id))}}" class="btn btn-info"><i
                    class="ri-eye-line"></i></a> --}}
                    @endif

                    <a href="{{ route('admin-InstagramSliders.edit', base64_encode($result->id)) }}" class="btn btn-info"><i
                            class="ri-edit-line"></i></a>

                    @if ($deletePermission == 1)
                        <form method="GET" action="{{ route('admin-InstagramSliders.delete', base64_encode($result->id)) }}">
                            @csrf
